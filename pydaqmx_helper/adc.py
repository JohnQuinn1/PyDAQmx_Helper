...
class ADC(Task):
    ...
    # def addChannel(self, channel,  ADC_mode=DAQmx_Val_RSE,  minRange=-10.0,  MaxRange=10.0):
    #    self.CreateAIVoltageChan((self.name+str(channel)).encode('utf-8'), b"", ADC_mode, minRange, MaxRange, DAQmx_Val_Volts, None)
    #   self.channelsNum = self.channelsNum + 1

    
    def sampleVoltages(self, nPointsPerChannel=1, sampleRate=1):
        """ Sample voltages at given rate from all channels,  default,  1,  1
        ...
        self.CfgSampClkTiming(b"", 
                              sampleRate, 
                              DAQmx_Val_Rising, 
                              DAQmx_Val_FiniteSamps, 
                              2 if nPointsPerChannel == 1 else nPointsPerChannel)
        self.StartTask()
        self.ReadAnalogF64(nPointsPerChannel, 
                           -1, 
...
#        sample = OrderedDict(zip(activeChannels, sample))
        sample = dict(zip(activeChannels, sample))
...
#            return OrderedDict(sorted(newSample.items(),  key=lambda t: channels.index(t[0])))
    ...
    def addChannels(self, newChannels, ADC_mode='DAQmx_Val_Diff', minRange=-10.0, maxRange=10.0):
        """ Add a list of channels with default settings or if given by user"""
        ADC_mode = eval(ADC_mode)
        for newChannel in newChannels:
            if(newChannel not in self.getActiveChannels()):
                self.CreateAIVoltageChan((self.name + str(newChannel)).encode('utf-8'), 
                                         b"", 
                                         ADC_mode, 
                                         minRange, 
                                         maxRange, 
